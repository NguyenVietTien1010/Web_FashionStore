...
// ========================= Đơn Hàng ===========================
function formatDate(dateString, stt) {
    var date = new Date(dateString);
    var year = date.getFullYear().toString().slice(-2); // Lấy 2 chữ số cuối của năm
    var month = (date.getMonth() + 1).toString().padStart(2, '0'); // Thêm số 0 vào tháng nếu nhỏ hơn 10
    var day = date.getDate().toString().padStart(2, '0'); // Thêm số 0 vào ngày nếu nhỏ hơn 10
    var sttPadded = (stt % 100).toString().padStart(2, '0'); // Đảm bảo số thứ tự có 2 chữ số, từ 00 đến 99

    return `DH-${day}-${month}-${year}-${sttPadded}`;
}
...
function addTableDonHang() {
    ...
    for (var i = 0; i < listDH.length; i++) {
        ...
        s += `<tr>
            <td style="width: 5%">` + (i+1) + `</td>
            <td style="width: 13%">` + formatDate(d.ma, i+1) + `</td> <!-- Sử dụng hàm formatDate với số thứ tự -->
...
function duyet(maDonHang, duyetDon) {
    ...
    for (var i = 0; i < u.length; i++) {
        for (var j = 0; j < u[i].donhang.length; j++) {
            if (u[i].donhang[j].ngaymua.toString() == maDonHang) {
                if (duyetDon) {
                    ...
                    if (u[i].donhang[j].tinhTrang == 'Đang chờ xử lý') {
                        if (window.confirm('Sau khi hủy sẽ không duyệt lại được!'))
...
function getValueOfTypeInTable_DonHang(tr, loai) {
    ...
    switch(loai) {
        case 'stt': return Number(td[0].innerHTML);
        case 'ma' : return new Date(td[1].innerHTML); // trả về dạng ngày để so sánh ngày
        case 'khach' : return td[2].innerHTML.toLowerCase(); // lấy tên khách
        case 'sanpham' : return td[3].children.length;    // lấy số lượng hàng trong đơn này, length: số lượng <p>
