var nameProduct, maProduct, sanPhamHienTai; // Tên sản phẩm là biến toàn cục, không cần lấy từ url nhiều lần

window.onload = function () {
    khoiTao();

    // thêm tags (từ khóa) vào khung tìm kiếm
    var tags = ["Uniqlo", "Dior", "Chanel", "Nike", "H&M"];
...
function phanTich_URL_chiTietSanPham() {
    ...
    // Đổi title
    document.title = nameProduct + ' - Fashion Store';
    ...
    var  s = addThongSo('Mô tả', sanPhamHienTai.detail.description);
    s += addThongSo('Màu sắc', sanPhamHienTai.detail.color);
    s += addThongSo('Kích cỡ', sanPhamHienTai.detail.sz);
    s += addThongSo('Chất liệu', sanPhamHienTai.detail.material);
    s += addThongSo('Kiểu dáng', sanPhamHienTai.detail.style);
    s += addThongSo('Tổng quan', sanPhamHienTai.detail.overview);
    s += addThongSo('Hướng dẫn giặt', sanPhamHienTai.detail.care);
    s += addThongSo('Size mẫu', sanPhamHienTai.detail.size);
    s += addThongSo('Thương hiệu', sanPhamHienTai.detail.brand);
    ...
    // Hình nhỏ
    addSmallImg("img/products/PD2.jpg");
    addSmallImg("img/products/PD2.jpg");
    addSmallImg("img/products/PD2.jpg");
    addSmallImg("img/products/PD2.jpg");
    addSmallImg("img/products/PD2.jpg");
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        case 'tragop':
            ...
            return `Khách hàng có thể mua trả góp sản phẩm với ` + span + `với thời hạn 2 tháng kể từ khi mua hàng.`;
        ...
        case 'moiramat':
            return `Khách hàng sẽ được mua với giá ưu đãi khi đặt trước tại cửa hàng. Đổi trả sản phẩm trong vòng 2 tháng.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi mua hàng online bằng thẻ VPBank hoặc thanh toán bằng Momo`;

        default:
            var span = `<span style="font-weight: bold">voucher trị giá 1.000.000đ</span>`;
            return `Cơ hội trúng ` + span + ` khi tham gia vòng quay may mắn tại cửa hàng`;
    }
}
...
function suggestion(){
    ...
    .map(sanPham => {
        ...
        // Tiêu chí 2: các thông số sản phẩm giống nhau
